      ...
      <div className="storyLayout">
        <article className="storyArticle">
          ...
          <section className="discussionPanel storyDiscussion">
            ...
            {comments.length === 0 ? <p className="storyEmptyComment">아직 첫 대화가 없어요. 궁금한 점을 가장 먼저 남겨 보세요.</p> : comments.map((comment) => <article key={comment.id} className={`discussionComment ${comment.kind.toLowerCase()}`}><div className="commentAvatar" aria-hidden>{comment.authorName.slice(0, 1)}</div><div><strong>{comment.authorName}</strong><small>{comment.kind === "APPLICATION" ? "참여 신청" : "질문"} · {formatDate(comment.createdAt, true)}</small><p>{comment.body}</p></div></article>)}
            {status === "authenticated" ? item.myApplicationStatus && item.myApplicationStatus !== "CANCELED" ? <button type="button" className="outlineButton" onClick={() => void cancel()}>참여 신청 취소 ({item.myApplicationStatus})</button> : <div className="discussionComposer storyComposer"><div className="composerKinds"><button type="button" aria-pressed={commentKind === "QUESTION"} onClick={() => setCommentKind("QUESTION")}>질문 남기기</button><button type="button" aria-pressed={commentKind === "APPLICATION"} onClick={() => setCommentKind("APPLICATION")}>참여 신청</button></div><textarea id="recruitment-discussion" value={commentBody} onChange={(event) => setCommentBody(event.target.value)} placeholder={commentKind === "APPLICATION" ? "함께하고 싶은 이유를 편하게 들려주세요." : "여행에 관해 궁금한 점을 남겨주세요."} /><button className="primaryButton" type="button" disabled={!isOpen && commentKind === "APPLICATION"} onClick={() => void postComment()}><Send size={15} aria-hidden /> {commentKind === "APPLICATION" ? "신청 보내기" : "질문 등록"}</button></div> : <Link className="outlineButton" href="/login">로그인 후 대화 참여하기</Link>}
          </section>
        </article>
        ...
        <aside className="storySidebar">
          <section className="tripSummaryCard">
            ...
            <dl><div><dt><CalendarClock size={17} aria-hidden /> 만남</dt><dd>{formatDate(item.meetingAt, true)}</dd></div><div><dt><MapPinned size={17} aria-hidden /> 노선</dt><dd>{item.routeLabel}</dd></div><div><dt><Users size={17} aria-hidden /> 모집 인원</dt><dd><b>{item.acceptedCount}</b> / {item.capacity}명</dd></div></dl>
          ...
          {!owner && status === "authenticated" && !item.myApplicationStatus ? <section className="storyApplyCard"><span>같은 방향의 여행을 찾고 있다면</span><b>이 모집에 합류해 보세요.</b><button type="button" disabled={!isOpen} onClick={() => { setCommentKind("APPLICATION"); document.getElementById("recruitment-discussion")?.focus(); }}><UserPlus size={17} aria-hidden /> 참여 신청하기</button></section> : null}
