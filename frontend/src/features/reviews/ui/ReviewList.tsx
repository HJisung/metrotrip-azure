...
export function ReviewList() {
  ...
  const fetchPage = useCallback(async (page: number, replace: boolean) => {
    ...
    setLoading(true);
    try {
      const response = await listReviews({ keyword: keyword || undefined, page, size: 12 });
  ...
  return (
    <PreviewFrame title="후기 게시판" description="다른 여행자들의 경험을 확인하고, 당신의 여행을 공유해보세요.">
      <div className="flex flex-wrap items-end justify-between gap-[var(--review-grid-gap)]">
        <div className="grid gap-xs"><span className="text-label-caps text-primary">METROTRIP REVIEWS</span><h2 className="text-headline-md font-heading text-on-surface">여행의 순간을 모아봤어요</h2></div>
        <Button onClick={() => navigate(getReviewPath({ kind: 'create' }))}>후기 작성하기</Button>
      </div>
      <form className="flex items-center gap-sm rounded-[var(--radius-lg)] border border-outline-variant/70 bg-surface-bright p-[var(--spacing-sm)] shadow-sm" onSubmit={(event) => { event.preventDefault(); setKeyword(keywordDraft.trim()); }}>
        <Search className="ml-sm shrink-0 text-on-surface-variant" size={18} />
        <Input className="border-0 shadow-none" value={keywordDraft} onChange={(event) => setKeywordDraft(event.target.value)} placeholder="여행지, 제목, 키워드로 검색" />
      </form>
