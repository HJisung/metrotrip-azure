...
| — | 경로 — 최단거리·최소환승 + 경유역 장소 추천 (SPEC 2-2) | ✅ 동작 (노선이 1호선뿐이라 환승은 0회) |
...
│  ├─ route-plan/              경로 Feature — 탐색·비교·경유역 타임라인
│  │  ├─ api/                  경로 조회 (나중에 GET /api/v1/routes 로 교체)
│  │  ├─ data/                 노선별 역 순서 (임시 — API 로 대체될 것)
│  │  ├─ hooks/                출발·도착 선택과 계산 상태
│  │  ├─ lib/                  노선 그래프 탐색 (최단거리·최소환승)
│  │  └─ ui/                   커버 헤더·경로 비교·경유역 타임라인
...
- 노선도/시간표/마이페이지의 **실제 동작** (지금은 화면만 — SPEC 2-1 참고)
- 경로 기능의 **전체 노선 데이터** — DB 담당이 API 로 제공하기로 협의됨.
  지금은 1호선 천안·아산 11개 역뿐이라 환승이 발생하지 않는다 (SPEC 2-2 참고)
